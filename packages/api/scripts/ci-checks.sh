...
# CI Checks Script for API Package
# This script runs all the quality checks needed for GitLab CI

set -e  # Exit on any error

echo "🚀 Starting CI checks for API package..."
...
# Change to API directory if not already there
...
if yarn prettier --check "src/**/*.ts" "test/**/*.ts"; then
...
# Run E2E tests if they exist and DATABASE_URL is set
if [[ -n "${DATABASE_URL}" ]]; then
    ...
    print_warning "DATABASE_URL not set, skipping E2E tests"
fi
